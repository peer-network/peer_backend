...
class AdvertisementMapper
{
    ...
    // Get all data cols from advertisements table belong to userid.
    public function fetchByID(string $userId, int $offset, int $limit): array
    {
        $this->logger->info("AdvertisementMapper.fetchByID started");

        $sql = "SELECT advertisementid, userid, status, timestart, timeend FROM advertisements_log WHERE userid = :userId ORDER BY status DESC, createdat DESC LIMIT :limit OFFSET :offset";
    ...
    public function isAdvertisementIdExist(string $advertisementId, string $status): bool
    {
        $this->logger->info("AdvertisementMapper.isAdvertisementIdExist started");

        $sql = "SELECT 1 FROM advertisements WHERE advertisementid = :advertisementId AND status = :status LIMIT 1";
        $stmt = $this->db->prepare($sql);
        $stmt->bindValue(':advertisementId', $advertisementId, \PDO::PARAM_STR);
    ...
    public function update(Advertisements $post): Advertisements
    {
        ...
        $query1 = "UPDATE advertisements 
                    ...
                    WHERE advertisementid = :advertisementid AND status = :status";

        $query2 = "INSERT INTO advertisements_log 
                    (advertisementid, userid, status, timestart, timeend) 
                    VALUES (:advertisementid, :userid, :status, :timestart, :timeend)";
        ...
        try {
            ...
            $stmt1->bindValue(':advertisementid', $data['advertisementid'], \PDO::PARAM_STR);
            ...
            $this->db->commit();
            $this->logger->info("Updated PostAdvertisement and inserted into Log");
            return new Advertisements($data);
        } catch (\Throwable $e) {
            $this->db->rollBack();
