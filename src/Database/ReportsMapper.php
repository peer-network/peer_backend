...
class ReportsMapper
{
    ...
    public function addReport(string $reporter_userid, ReportTargetType $targettype, string $targetid, string $message): bool
    {
        ...
        $debugData = ['reporter_userid' => $reporter_userid, 'targetid' => $targetid,'targettype' => $targetTypeString];
        
        try {
            $this->db->beginTransaction();

            // Check if the record already exists
            $sqlCheck = "SELECT COUNT(*) FROM user_reports WHERE reporter_userid = :reporter_userid AND targetid = :targetid AND targettype = :targettype";
            ...
            if (!$exists) {
                ...
                // Insert a new record
                $sql = "INSERT INTO user_reports (reportid, reporter_userid, targetid, targettype, collected, createdat) VALUES (:reportid, :reporter_userid, :targetid, :targettype, :collected, :createdat)";
