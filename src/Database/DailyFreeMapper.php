...
class DailyFreeMapper
{
    ...
    public function incrementUserDailyUsage(string $userId, int $artType): bool
    {
        ...
        try {
            ...
            $existsQuery = "SELECT createdat FROM dailyfree WHERE userid = :userId AND createdat::date = CURRENT_DATE";
            ...
            $recordDate = $result['createdat'] ?? null;

            if ($result) {
                $isToday = $recordDate && (new \DateTime($recordDate))->format('Y-m-d') === date('Y-m-d');

                if ($isToday) {

                    $updateQuery = "UPDATE dailyfree SET $column = $column + 1 WHERE userid = :userId AND createdat::date = CURRENT_DATE";
                    ...
                    $resetQuery = "UPDATE dailyfree 
                                   SET liken = 0, comments = 0, posten = 0, createdat = NOW() 
                                   WHERE userid = :userId";
                    $resetStmt = $this->db->prepare($resetQuery);
                    $resetStmt->bindValue(':userId', $userId, \PDO::PARAM_STR);
                    $resetStmt->execute();

                    $incrementQuery = "UPDATE dailyfree SET $column = 1 WHERE userid = :userId AND createdat::date = CURRENT_DATE";
                    $incrementStmt = $this->db->prepare($incrementQuery);
                    $incrementStmt->bindValue(':userId', $userId, \PDO::PARAM_STR);
                    $success = $incrementStmt->execute();
                }
            } else {
                ...
                $insertStmt->bindValue(':liken', $artType === LIKE_ ? 1 : 0, \PDO::PARAM_INT);
                $insertStmt->bindValue(':comments', $artType === COMMENT_ ? 1 : 0, \PDO::PARAM_INT);
                $insertStmt->bindValue(':posten', $artType === POST_ ? 1 : 0, \PDO::PARAM_INT);
