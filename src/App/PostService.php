        ...
        try {
            ...
            elseif ($args['contenttype'] === 'video' && !empty($mediaPath['path'])) {
                try {
                    ...
                    $generator = new \Fawaz\Services\VideoCoverGenerator();
                    $generatedCoverPath = $generator->generate($videoFilePath);

                    $binary = file_get_contents($generatedCoverPath);
                    ...
                    if (is_file($generatedCoverPath)) {
                        unlink($generatedCoverPath);
                        $this->logger->debug('Temporary cover file deleted after use', ['path' => $generatedCoverPath]);
                    }

                } catch (\Throwable $e) {
                    $this->logger->error('Autogenerated cover generation error: ' . $e->getMessage());

                    if (isset($generatedCoverPath) && is_file($generatedCoverPath)) {
                        unlink($generatedCoverPath);
