...
class AdvertisementService
{
    ...
    public function resolveAdvertisePost(?array $args = []): ?array
    {
        ...
        if (false === $this->postService->postExistsById($postId)) {
        ...
        if ($advertisePlan === $this::PLAN_BASIC) {
            ...
            // Laufzeit validieren
            if (null !== $durationInDays && !\in_array($durationInDays, $durationActions, true)) {
        ...
        if (true === $this->isAdvertisementDurationValid($postId)) {
            $reducePrice = true;
        }

        if (false === $reducePrice) {
            if (true === $this->hasShortActiveAdWithUpcomingAd($postId)) {
                $reducePrice = true;
            }
        }
        ...
        if ($advertisePlan === $this::PLAN_PINNED) {
            ...
            if (true === $reducePrice) {
                $CostPlan -= ($CostPlan * 0.20); // 80% vom ursprünglichen Wert
        ...
        if (empty($CostPlan) || 0 === (int) $CostPlan) {
    ...
    public static function calculatePrice(string $plan, ?string $duration = null): int
    {
        ...
        if (self::PLAN_PINNED === $plan) {
            return (int) $pricePinned;
        }

        if (self::PLAN_BASIC === $plan) {
            if (null === $duration) {
    ...
    private function formatStartAndEndTimestamps(\DateTimeImmutable $startDate, string $durationKey): array
    {
        ...
        } while ('0' === $firstDigit || '0' === $lastDigit || '1' === $lastDigit);
    ...
    public function createAdvertisement(array $args = []): array
    {
        ...
        try {
            if (null !== $CostPlan && self::PLAN_BASIC === $CostPlan) {
                if ($startday) {
                ...
                if (true === $this->advertisementMapper->hasTimeConflict($postId, strtolower($CostPlan), $timestart, $timeend, $this->currentUserId)) {
            ...
            } elseif (null !== $CostPlan && self::PLAN_PINNED === $CostPlan) {
                if (true === $this->advertisementMapper->hasActiveAdvertisement($postId, strtolower($CostPlan))) {
            ...
            } elseif (self::PLAN_PINNED === $CostPlan) {
    ...
    public function fetchAll(?array $args = []): array
    {
        ...
        if (null !== $to && !self::validateDate($to)) {
            return self::respondWithError(30213);
        }

        if (null !== $advertisementtype && !\in_array($advertisementtype, $advertiseActions, true)) {
            return $this->respondWithError(32006);
        }

        if (null !== $advertisementId && !self::isValidUUID($advertisementId)) {
            return $this->respondWithError(30269);
        }

        if (null !== $advertisementId && !$this->advertisementMapper->advertisementExistsById($advertisementId)) {
        ...
        if (null !== $postId && !$this->postMapper->postExistsById($postId)) {
            return $this->respondWithError(31510);
        }
        ...
        if (null !== $userId && !$this->userMapper->isUserExistById($userId)) {
