...
class MintAccountRepositoryImpl implements MintAccountRepository
{
    ...
    {
        $this->logger->debug('MintAccountRepositoryImpl.getDefaultAccount started');
        ...
        if (!$row) {
            $this->logger->warning('MintAccountRepositoryImpl.getDefaultAccount: no rows');
            return null;
        }
    ...
    {
        $this->logger->debug('MintAccountRepositoryImpl.deductCurrentBalance started', [
            'accountId' => $userId,
            'amount' => $amount,
        ]);
        ...
        try {
            ...
            $this->logger->info('MintAccountRepositoryImpl.deductCurrentBalance succeeded', [
                'accountId' => $userId,
                'amount' => $amount,
            ...
            $this->logger->error('MintAccountRepositoryImpl.deductCurrentBalance failed', [
                'error' => $e->getMessage(),
            ]);
            throw $e;
        }
    ...
    {
        $this->logger->debug('MintAccountRepositoryImpl.lockWalletBalance started', [
            'walletId' => $walletId,
        ]);
