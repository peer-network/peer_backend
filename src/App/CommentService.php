...
class CommentService
{
    ...
    private function validateRequiredFields(array $args, array $requiredFields): array
    {
        foreach ($requiredFields as $field) {
            if (empty($args[$field])) {
                return $this::createResponse(30265);
            }
        }
        return [];
    }
    ...
    public function createComment(?array $args = []): array
    {
        ...
        if (!$this->validateUUID($postId)) {
            return $this::createResponse(30209, ['postid' => $postId]);
        }

        if ($parentId !== null && !$this->validateUUID($parentId)) {
			return $this::createResponse(31603, ['parentId' => $parentId]);
        }
        ...
        if ($parentId !== null) {
            if (!$this->commentMapper->isParentTopLevel($parentId)) {
                return $this::createResponse(41604);
            }
        }
        ...
        try {
            ...
            if (empty($commentId)) {
                ...
                return $this::createResponse(41607);
            }
            ...
            if (!$result) {
                ...
                return $this::createResponse(41602);
            }
