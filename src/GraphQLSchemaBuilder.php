...
class GraphQLSchemaBuilder
{
    ...
    public function getQueriesDependingOnRole(): ?string
    {
        ...
        if (null !== $this->currentUserId) {
            if (Role::USER === $this->userRoles) {
                $schema = $userSchema;
            } elseif (Role::WEB3_BRIDGE_USER === $this->userRoles) {
                $schema = $bridgeSchema;
            } elseif (Role::ADMIN === $this->userRoles) {
                $schema = $adminSchema;
            } elseif (Role::MODERATOR === $this->userRoles) { // Role::MODERATOR
    ...
    {
        if (null !== $bearerToken && '' !== $bearerToken) {
            try {
    ...
    public function buildResolvers(): array
    {
        return [
            ...
            'Advertisement' => [
                ...
                'amountReports'  => fn (array $root): int => $root['amountreports'] ?? 0,
                'createdAt'      => fn (array $root): string => $root['createdat'] ?? '',
                'user'           => fn (array $root): array => // neu
                    $root['user'] ?? [],
                'post' => fn (array $root): array => // neu
                    $root['post'] ?? [],
            ],
            ...
            'TargetContent' => [
                'post'    => fn (?array $root): ?array => $root['post'] ?? null,
                'comment' => fn (?array $root): ?array => $root['comment'] ?? null,
                'user'    => fn (?array $root): ?array => $root['user'] ?? null,
            ],
        ];
    }
    ...
    protected function resolveComments(array $args): array
    {
        ...
        if (isset($comments['status']) && 'error' === $comments['status']) {
            return $comments;
        }
    ...
    protected function resolveTags(array $args): ?array
    {
        ...
        if (isset($tags['status']) && 'success' === $tags['status']) {
            $this->logger->info('Query.resolveTags successful');

            return $tags;
        }

        if (isset($tags['status']) && 'error' === $tags['status']) {
            return $tags;
        }
    ...
    protected function resolveLiquidity(): ?array
    {
        ...
        if (isset($results['status']) && 'error' === $results['status']) {
            return $results;
        }
    ...
    protected function resolvePosts(array $args): ?array
    {
        ...
        if (isset($posts['status']) && 'error' === $posts['status']) {
            return $posts;
        }
    ...
    protected function ContactUs(?array $args = []): array
    {
        ...
        try {
            $contact = new App\Contactus($args);
