...
class JSONHandler
{
    ...
    {
        $jsonString = self::generateJson($data, $name, $addHash);

        if (false === file_put_contents($outputPath, $jsonString)) {
    ...
    public static function parseInputJson(string $filePath, bool $validateKeyUniqness = false): mixed
    {
        ...
        if (true == $validateKeyUniqness) {
            $duplications = self::getDuplicatedNumericKeys($jsonContent);
